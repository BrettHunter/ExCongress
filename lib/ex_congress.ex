defmodule ExCongress do
  use HTTPoison.Base
  alias ExCongress.Client
  @moduledoc """
  ExCongress is an Elixir wrapper for the [Sunlight Labs Congress API]
  ...
  @user_agent [{"User-agent", "ExCongress"}]
    ...
    defp pagination(options) do
      Keyword.get(options, :pagination,
        Application.get_env(:ex_congress, :pagination, nil))
    end
    ...
    @doc """
    Adds the API key to as X-APIKEY Header.
    ## Example
        iex> ExCongress.authorization_header(%{api_key: "${API_KEY}"}, [])
        [{"X_APIKEY", "1234abcd1234abcd1234abcd"}]
    ## More info
    https://sunlightlabs.github.io/congress/
    """
    @spec authorization_header(Client.auth, list) :: list
    def authorization_header(%{api_key: token}, headers) do
